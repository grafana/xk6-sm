...
func TestAll(t *testing.T) {
	...
	getBuildInfo = sync.OnceValue(func() *debug.BuildInfo {
		...
		if _, found := slices.BinarySearchFunc(buildinfo.Settings, revisionKey, isKey); !found {
			buildinfo.Settings = append(buildinfo.Settings,
				debug.BuildSetting{Key: revisionKey, Value: revisionValue})
		}

		if _, found := slices.BinarySearchFunc(buildinfo.Settings, timeKey, isKey); !found {
			buildinfo.Settings = append(buildinfo.Settings,
				debug.BuildSetting{Key: timeKey, Value: timeValue})
		}
