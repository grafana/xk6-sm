...
// It can be used as a map key because [metrics.TimeSeries] cans. This relies on k6's promise to keep [metrics.TagSet]
// immutable, and always reuse the same TagSet instance for each given metric.
...
// timeseriesFromK6 creates a timeseries from [metrics.TimeSeries].
...
// metricStore temporarily stores and processes k6 samples according to SM needs. It performs on-the-fly sample
// aggregation, and post-processing when k6 is done executing.
//
// Post-processing does four things:
// - Derive new timeseries from existing ones, in a 1:N mapping
...
// - Promql-like aggregations (by label)
// - N:M or N:1 metric mappings
//
// metricStore does not handle threshold reporting.
//
...
// Record aggregates a new sample into the store in a thread-safe way.
func (ms *metricStore) Record(sample metrics.Sample) {
	...
	log.Debugf("Aggregating sample (%f) for %q into existing (%f) value", sample.Value, ts.name, old.value)
	...
	case metrics.Gauge:
		...
		updated.value = sample.Value
	default:
		log.Debugf("Unknown metric type %q for %q, keeping previous sample without aggregating", ts.metricType, ts.name)
		return
	}
	...
	for ts, v := range ms.store {
		...
		case "http_reqs":
			func() {
				...
				log.Debugf("Created %q from %q", renamedTS.name, ts.name)
			}()
			...
			func() {
				...
				log.Debugf("Created %q from %q", infoTS.name, ts.name)
			}()
			...
			func() {
				...
				log.Debugf("Created %q from %q", sslTS.name, ts.name)
			}()
			...
			func() {
				...
				log.Debugf("Created %q from %q", responseTS.name, ts.name)
			}()
			...
			func() {
				...
				log.Debugf("Created %q from %q", errorCodeTS.name, ts.name)
			}()
		...
		case "data_sent", "data_received":
			...
			log.Debugf("Created %q from %q", wihtSuffixTS.name, ts.name)
...
func (ms *metricStore) RemoveLabels() {
	...
	for ts, v := range ms.store {
		if ts.name != "http_info" {
			...
			log.Debugf("Removing http info labels from %q", ts.name)
		...
		if group, found := ts.tags.Get("group"); found && group == "" {
			log.Debugf("Removing empty group label from %q", ts.name)
